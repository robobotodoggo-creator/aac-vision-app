...
## Project Status
All HIGH and MEDIUM priority items complete. Top five LOW items (custom symbol creation, export/import config, usage analytics, dark/light theme toggle, sound effects) also done. `flutter analyze` passes clean. APK build not verified (no Java runtime on this machine). App has NOT been tested on a physical device yet.
...
3. **Onboarding tutorial overlay** — Next LOW priority item

## Key Files Changed This Shift
- `lib/services/sound_service.dart` — New file; generates 50ms click WAV programmatically (800Hz sine burst with exponential decay, 22050Hz 16-bit mono PCM); plays via `audioplayers` `BytesSource`
- `lib/services/app_state.dart` — Added `SoundService sound` field, `_soundEffects` toggle with `setSoundEffects()` and SharedPreferences persistence; `sound.init()` in parallel startup; `sound.playTap()` in `speakSymbol()` and `addToSentence()`; `sound.dispose()` in dispose
- `lib/screens/settings_screen.dart` — Added Tap Sound Effects SwitchListTile in Appearance section (volume_up/volume_off icon)
- `pubspec.yaml` — Added `audioplayers: ^6.6.0` dependency
