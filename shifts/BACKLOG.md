...
- [x] **Custom AAC-trained TFLite vision model:** ML Kit's default ~400-class image labeler misses the long tail of AAC-relevant objects. Research complete: cataloged 500 AAC-relevant classes across 11 domains, identified dataset sources (OpenImages V7 covers ~120, ~330 need new data), designed EfficientNet-Lite0 INT8 training pipeline (~5 MB model, ~6.5 ms latency), designed per-user personalization concept (base+head architecture, on-device training). Full report: `docs/research/custom-tflite-model.md`. (added 2026-05-28, completed 2026-05-28)
